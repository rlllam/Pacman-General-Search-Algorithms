...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.Corner(self.startingPosition, self.corners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                corner_copy = copy.deepcopy(state)
                corner_copy.touch(nextState)
                successors.append((corner_copy, action, 1))
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    foodCoors = foodGrid.asList()
    
    if foodCoors:
        max = 0
        
        for coor in foodCoors:
            distance = mazeDistance(coor, position, problem.startingGameState)
            if distance > max:
                max = distance
